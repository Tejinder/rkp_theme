...
function initiateRTEWithHeight(textareaid, characterLimit, setPlainText, contentHeight, callback)
{	
...
//        toolbar2: "forecolor backcolor",		
      ...
      /*  templates: [
		...
		*/
		...
		//	resizeTextEditorArea(editor.id+'_ifr');
			});
            editor.on('change', function(e){
                console.log("Working...");
                 $j("#"+textareaid).val(editor.getContent());
...
function resizeTextEditorArea(iFrameName)
{
	var x = document.getElementById(iFrameName);
	console.log(iFrameName + " x  == " + x)
	if(x!=null)
	{
		var y = x.contentDocument;
		console.log(iFrameName + " y   == " + y)
		//var m = y.body.style.height;
		if(y!=null)
		{
			var m = y.body.clientHeight;

			console.log(iFrameName + " m   == " + y.body.clientHeight)
			//document.getElementById("researchQuestion_ifr").style.height = m;
			if(m!=null)
			{
				var the_height= document.getElementById(iFrameName).contentWindow.document.body.offsetHeight ;
				console.log(iFrameName + " the_height   == " + the_height)
				document.getElementById(iFrameName).style.height = the_height+'px';
			}
		}
	}
	
	//$j("#researchQuestion_ifr").css("height", 123);	
}
