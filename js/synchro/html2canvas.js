...
_html2canvas.Parse = function (images, options) {
  ...
  function renderListItem(element, stack, elBounds) {
    ...
    if (/^(decimal|decimal-leading-zero|upper-alpha|upper-latin|upper-roman|lower-alpha|lower-latin|lower-roman)$/i.test(type)) {
  ...
  function renderFormValue (el, bounds, stack){
    ...
    if (/^(submit|reset|button|text|password)$/.test(el.type) || el.nodeName === "SELECT"){
	 ...
	 // valueWrap.style.paddingBottom = "10px";
    }
	
	valueWrap.style.top = bounds.top + "px";
	
	if (/^(text)$/.test(el.type) && !isIEBrowser())
	 {
		valueWrap.style.top = (bounds.top-7) + "px";
	 }
	 else
	 {
		valueWrap.style.top = bounds.top + "px";
