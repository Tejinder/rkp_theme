...
jive.UserPicker.Source = jive.oo.Class.extend(function(protect) {
    ...
    this.autocomplete = function(query) {
        ...
        var params = {
            ...
            propNames: ['avatarID', 'hasConnection', 'title', 'department'],
			srole: that.options.$input.attr("srole")
        };

        if (that.options.object) {
            params = $j.extend(params, {
                objectID: that.options.object.objectID || that.options.object.id,
...
define('apps/userpicker/models/userpicker_source', function() {
    return jive.UserPicker.Source;
});