<div id="user_doc_drilldown_report" name="user_doc_drilldown_report">
   <#if drillDownMapReport?? && drillDownMapReport?size &gt; 0>
	...
	<#assign reportType = drillDownMapReport.get('type')?string />
	<#assign activityType = drillDownMapReport.get('activity')?string />
	<#assign doc_id = drillDownMapReport.get('obj_id')?c />
	<#assign act_type = drillDownMapReport.get('act_type')?string />
	...
	<div class='drilled_report_data'>
		...
		<#assign eventDetailsList = drillDownMapReport.get('dataList')/>
			...
			<tbody>
				...
				<tr>
					...
					<td style='font-weight:bold;'>
						${drillDownMapReport.get('total_act_count')?string}
					</td>
				</tr>
 ...
 <#macro ajxExportOption>
	...
	<div id="export_drill_down_report" style="padding-top:10px;">
		<span>Export this report :
			 <select name="exportType" id="exportType" onchange="downloadDrillDownReport(this);">
        ...
        <td style="text-align:right;">
	      ...
	      <@s.text name="global.pages" /><@s.text name="global.colon" />
                ...
                <#if dropDownRequired>
                    <span>Go to page :
                        <#assign pages = newPaginator.getPages()>
                        <select name="goto" id="goto" onchange="javascript:fetchDocumentDrillDownPage(this.value, false);">
                ...
                <#if (newPaginator.previousPage)>
                    <a href="javascript:fetchDocumentDrillDownPage('${newPaginator.previousPageStart?c}', false);">
                ...
                <#list 0..pages?size-1 as idx>
                    ...
                    <#else>
                        <a href="javascript:fetchDocumentDrillDownPage('${pages[idx].start?c}', false);"
                           class="<#if (newPaginator.start?c == pages[idx].start?c)>jive-current</#if>">${pages[idx].number}</a>
                     </#if>
                </#list>

                <#if (newPaginator.nextPage)>
                    | <a href="javascript:fetchDocumentDrillDownPage('${newPaginator.nextPageStart?c}', false);">
...
<#macro numResultsSelect eleName='ajxNumResults' startIndex=10?c>
	<select name='${eleName}' id='${eleName}' onchange="javascript:fetchDocumentDrillDownPage(this.value, true);">
