...
<script type="text/javascript">
	...
	function showProjectSpecsVal() 
	{
        dialog({title:"Message",html:"The Project Specs for the market are not approved. Report section will be accessible as soon as the approval is done."});
		return false;
    ...
    function confirmDelete(attachmentId, fieldID, attachmentName) {
          location.href="/synchro/report-multi-summary!removeAttachment.jspa?projectID=${projectID?c}&endMarketId=${endMarketId?c}&attachmentId="+attachmentId+"&attachmentFieldID="+fieldID+"&attachmentName="+attachmentName;
        /* $j.ajax({
       url: "<@s.url value='/synchro/report-multi-summary!removeAttachment.jspa'/>",
            type: 'POST',
...
<#assign isProcurementUser = statics['com.grail.synchro.util.SynchroPermHelper'].isProcurementUser(projectID,statics['com.grail.synchro.SynchroConstants'].ABOVE_MARKET_MULTI_MARKET_ID) />
<#assign isCommunicationAgencyUser = statics['com.grail.synchro.util.SynchroPermHelper'].isCommunicationAgencyUser(projectID,statics['com.grail.synchro.SynchroConstants'].ABOVE_MARKET_MULTI_MARKET_ID) />
...
<#assign isSystemAdmin = statics['com.grail.synchro.util.SynchroPermHelper'].isSynchroAdmin(user) />
...
<div class="main-sub-menus-div endmarket-menus">
    ...
    <#list reportSummaryList as emd>

        <#assign psInitiation = jiveContext.getSpringBean("projectSpecsManager").getProjectSpecsInitiation(projectID,emd.getEndMarketID()).get(0) />
        <#if emd.getEndMarketID() == statics['com.grail.synchro.SynchroConstants'].ABOVE_MARKET_MULTI_MARKET_ID>
            <#if psInitiation.isApproved?? && psInitiation.isApproved==1 >
                <li><a href="/synchro/report-multi-summary!input.jspa?projectID=${projectID?c}" <#if (!endMarketId?exists) || (endMarketId &lt;= 0)>class="active"</#if>>Above Market</a></li>
            <#else>
                <li><a href="javascript:void(0);" <#if (!endMarketId?exists) || (endMarketId &lt;= 0)>class="active"</#if>>Above Market</a></li>
            </#if>
        <#else>
            <#if psInitiation.isApproved?? && psInitiation.isApproved==1 >
                <li><a href="/synchro/report-multi-summary!input.jspa?projectID=${projectID?c}&endMarketId=${emd.getEndMarketID()}" <#if endMarketId?exists && endMarketId == emd.getEndMarketID()>class="active"</#if>>${statics['com.grail.synchro.SynchroGlobal'].getEndMarkets().get(emd.getEndMarketID()?int)}</a></li>
            <#else>
               <#-- <li><a href="javascript:void(0);" onclick="showProjectSpecsVal()" <#if endMarketId?exists && endMarketId == emd.getEndMarketID()>class="active"</#if>>${statics['com.grail.synchro.SynchroGlobal'].getEndMarkets().get(emd.getEndMarketID()?int)}</a></li> -->
               <li><a href="/synchro/report-multi-summary!input.jspa?projectID=${projectID?c}&endMarketId=${emd.getEndMarketID()}"  <#if endMarketId?exists && endMarketId == emd.getEndMarketID()>class="active"</#if>>${statics['com.grail.synchro.SynchroGlobal'].getEndMarkets().get(emd.getEndMarketID()?int)}</a></li>
            </#if>
        </#if>
    </#list>
...
<form name="report-summary" id="report-summary-form" action="/synchro/report-multi-summary!execute.jspa" method="POST" name="form-create" class="research_pib pib" >
   ...
   <#-- <#elseif (reportSummaryInitiation.status?? && reportSummaryInitiation.status==statics['com.grail.synchro.SynchroGlobal$StageStatus'].REPORT_SUMMARY_COMPLETED.ordinal()) || (reportSummaryInitiation.isSPIApproved?? && reportSummaryInitiation.isSPIApproved && reportSummaryInitiation.legalApproval?? && reportSummaryInitiation.legalApproval)>-->
    <#elseif (reportSummaryInitiation.status?? && reportSummaryInitiation.status==statics['com.grail.synchro.SynchroGlobal$StageStatus'].REPORT_SUMMARY_COMPLETED.ordinal()) || (reportSummaryInitiation.isSPIApproved?? && reportSummaryInitiation.isSPIApproved )>
    ...
    <#elseif isProcurementUser || isCommunicationAgencyUser || projectContactHasReadonlyAccess>
        <@renderReportSummaryCheckboxes label="Upload Report/Oracle Summary for SP&I Approval" readonly=true />
    <#elseif !(isAboveMarketProjectContact || isProjectOwner || isCountryProjectContact || isCountrySPIContact || isExternalAgencyUser) >
        <@renderReportSummaryCheckboxes label="Upload Report/Oracle Summary for SP&I Approval" readonly=true />
    <#elseif !(isAboveMarket) && !(isCountryProjectContact || isCountrySPIContact || isExternalAgencyUser || isProjectOwner) >
        <@renderReportSummaryCheckboxes label="Upload Report/Oracle Summary for SP&I Approval" readonly=true />
    ...
    <#else>
        ...
        <div class="region-inner">
            ...
            <#if isAdminUser>
                ...
                <@macroCustomFieldErrors msg="Please enter Comments"/><br>
               ...
               <#-- <div class="character-limit" >You have <input readonly type="text" id="commentscountdown" name="countdown" size="3" value="1500" style="margin-top:7px;"> characters left</div> -->
                ...
                <#elseif !((reportSummaryInitiation.status?? && reportSummaryInitiation.status==statics['com.grail.synchro.SynchroGlobal$StageStatus'].REPORT_SUMMARY_COMPLETED.ordinal()) || (reportSummaryInitiation.isSPIApproved?? && reportSummaryInitiation.isSPIApproved ))>
                    ...
                    <#if !projectContactHasReadonlyAccess>
                        <#if !(isAboveMarket) && !(isCountryProjectContact || isCountrySPIContact || isExternalAgencyUser || isProjectOwner) >
                        <#elseif !(isAboveMarket) && endMarketStatus?? && endMarketStatus==statics['com.grail.synchro.SynchroGlobal$ProjectActivationStatus'].CANCEL.ordinal() >
                        <#elseif isAboveMarketProjectContact || isProjectOwner || isCountryProjectContact || isCountrySPIContact || isExternalAgencyUser >
                            <span class="jive-icon-med jive-icon-attachment j-link" onclick="showAttachmentPopup(${commentsID?c}, true)" ></span>
                        </#if>
...
<div id="emailNotification" style="display:none">
    <form id="email-notification-form" action="<@s.url value="/synchro/report-multi-summary!sendNotification.jspa"/>" method="post" enctype="multipart/form-data" class="j-form-popup">
...
<form method="POST" name="moveToNextStageForm" id="moveToNextStageForm" action="/synchro/report-multi-summary!moveToNextStage.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
    <input type="hidden" name="endMarketId" value="${endMarketId?c}">
...
<script type="text/javascript">
    ...
    $j(document).ready(function(){
        attachmentWindow = new SYNCHRO_FIELD_ATTACHMENT_POPUP({
            formUrl:"<@s.url value='/synchro/report-multi-summary!addAttachment.jspa'/>",
            projectID:${projectID?c},
...
<div class="pib-report-requirement">
    <div>
        <div class="check-field report-check-field">
            ...
            <span class="label">Full Report<#if isAboveMarket><lable style class="red">*</lable></#if></span>
        <#--<#if !(reportSummaryInitiation.isSPIApproved?? && reportSummaryInitiation.isSPIApproved && reportSummaryInitiation.legalApproval?? && reportSummaryInitiation.legalApproval)>-->
            <#if isAdminUser>
            ...
            <#--<#if !(attachmentMap?? && attachmentMap.get(fullReportID)?? && attachmentMap.get(fullReportID)?size &gt; 0)>-->
                <#if !projectContactHasReadonlyAccess>
                    <#if !(isAboveMarket) && !(isCountryProjectContact || isCountrySPIContact || isExternalAgencyUser) >
                    <#elseif !(isAboveMarket) && endMarketStatus?? && endMarketStatus==statics['com.grail.synchro.SynchroGlobal$ProjectActivationStatus'].CANCEL.ordinal() >
                    <#elseif isAboveMarketProjectContact || isProjectOwner || isCountryProjectContact || isCountrySPIContact || isExternalAgencyUser >
                        <span class="jive-icon-med jive-icon-attachment" onclick="showAttachmentPopup(${fullReportID?c}, true)"></span></td>
                    </#if>
    ...
    <div>
        <div class="check-field report-check-field width100">
            <#if reportSummaryInitiation.summaryForIRIS?? && reportSummaryInitiation.summaryForIRIS>
                <input type="checkbox"  name="summaryForIRIS-display" checked="true" disabled />
            ...
            <span class="label">Summary for IRIS<#if isAboveMarket><lable style class="red">*</lable></#if></span>
...
<div id="summaryForIRISWindow" style="display:none">
    <div class="summaryForIRIS view_edit_pib">
        <form id="synchro-pit-form" action="<@s.url value="/synchro/report-multi-summary!updateSynchroToIRIS.jspa"/>" method="post" class="j-form-popup">
	            ...
	            <div class="pib_band_view">
                    ...
                    <ul>
                        <li>
                            ...
                            <input type="text" name="sampleSize" id="sampleSize" class="error" <#if synchroToIRIS.sampleSize??> value="${synchroToIRIS.sampleSize?default('')?html}" <#else>value=""</#if> <#if reportSummaryInitiation.status?? && reportSummaryInitiation.status==statics['com.grail.synchro.SynchroGlobal$StageStatus'].REPORT_SUMMARY_COMPLETED.ordinal()> disabled </#if> />
...
<!-- Audit Logs -->
	...
	var projName = "${project.name?js_string}";
	SynchroLogUtil.addLogActivity("${statics['com.grail.synchro.SynchroGlobal$PortalType'].SYNCHRO.getDescription()}", ${statics['com.grail.synchro.SynchroGlobal$PageType'].PROJECT.getId()}, ${statics['com.grail.synchro.SynchroGlobal$Activity'].VIEW.getId()}, ${statics['com.grail.synchro.SynchroGlobal$LogProjectStage'].REPORT_SUMMARY.getId()}, "${i18CustomText}", projName, ${projectID?c}, ${user.ID?c}, ${endMarketId?c});
