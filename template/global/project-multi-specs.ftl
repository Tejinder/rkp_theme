...
<script type="text/javascript">
    ...
    {	
		...
		<#list psEMList as psEMDetail>
			<#assign emid = psEMDetail.endMarketID />
			initiateRTE('latestFWComments_${emid}', 1500);	
			initiateRTE('finalCostComments_${emid}', 1500);
		</#list>
    }
    ...
    function confirmDelete(attachmentId, fieldID, attachmentName) {
        location.href="/synchro/project-multi-specs!removeAttachment.jspa?projectID=${projectID?c}&endMarketId=${projSpecsEndMarketId?c}&attachmentId="+attachmentId+"&attachmentFieldID="+fieldID+"&attachmentName="+attachmentName;
        /*
        $j.ajax({
            url: "<@s.url value='/synchro/project-multi-specsremoveAttachment.jspa'/>",
            type: 'POST',
...
<#assign isProcurementUser = statics['com.grail.synchro.util.SynchroPermHelper'].isProjectOwner(projectID) />
<#assign isCommunicationAgencyUser = statics['com.grail.synchro.util.SynchroPermHelper'].isProjectOwner(projectID) />
...
<div class="main-sub-menus-div endmarket-menus">
    <ul class="main-sub-menus">
        <li><a href="/synchro/project-multi-specs!input.jspa?projectID=${projectID?c}" <#if (!endMarketId?exists) || (endMarketId &lt;= 0)>class="active"</#if>>Above Market</a></li>
    <#list psEMList as emd>
        <li><a href="/synchro/project-multi-specs!input.jspa?projectID=${projectID?c}&endMarketId=${emd.getEndMarketID()}" <#if endMarketId?exists && endMarketId == emd.getEndMarketID()>class="active"</#if>>${statics['com.grail.synchro.SynchroGlobal'].getEndMarkets().get(emd.getEndMarketID()?int)}</a></li>
    </#list>
    </ul>
</div>

<#assign projectInitiationList = jiveContext.getSpringBean("pibManager").getPIBDetails(projectID, statics['com.grail.synchro.SynchroConstants'].ABOVE_MARKET_MULTI_MARKET_ID) />
<#if projectInitiationList?? && (projectInitiationList?size>0)>
	<#assign projectInitiation = projectInitiationList.get(0) />
</#if>


<#assign canEditProject = statics['com.grail.synchro.util.SynchroPermHelper'].canEditProjectByStatus(projectID) />

<!-- Project Contact access check based on role hierarchies -->
...
<#if !isAdminUser && projectSpecsInitiation.status ?? && projectSpecsInitiation.status == statics['com.grail.synchro.SynchroGlobal$StageStatus'].PROJECT_SPECS_COMPLETED.ordinal() >
    <#include "/template/global/project-multi-specs-readonly.ftl" />
<#elseif !isAdminUser &&  projectSpecsInitiation.isApproved?? && projectSpecsInitiation.isApproved==1 && projectSpecsInitiation.approver?? >
	 <#include "/template/global/project-multi-specs-readonly.ftl" />
<#--
...
<#elseif projectContactHasReadonlyAccess>
    <#include "/template/global/project-multi-specs-readonly.ftl" />	
<#elseif !(isAboveMarketProjectContactUser || isProjectOwner || isCountryProjectContact || isCountrySPIContact || isExternalAgencyUser || isAdminUser) >
	<#include "/template/global/project-multi-specs-readonly.ftl" />
<#elseif !(isAboveMarket) && !(isCountryProjectContact || isCountrySPIContact || isExternalAgencyUser || isAdminUser || isProjectOwner) >
	<#include "/template/global/project-multi-specs-readonly.ftl" />
<#elseif !(isAboveMarket) && !isAdminUser &&  endMarketStatus?? && endMarketStatus==statics['com.grail.synchro.SynchroGlobal$ProjectActivationStatus'].CANCEL.ordinal() >
	<#include "/template/global/project-multi-specs-readonly.ftl" />
<#else>
...
<#if !(isExternalAgencyUser || synchroPermHelper.isExternalAgencyUser(user)) >
    ...
    <#if isProjectOwner || isAboveMarketProjectContactUser || isAdminUser || isGlobalSuperUser > 
	    ...
	    <#--<input type="text" name="name" value="${endMarketDetails.get(0).initialCost}" size="30" class="form-text-div numericfield numericformat longField" disabled="true">-->
	         <input type="text" name="name" value="${project.totalCost?default('')}" size="30" class="form-text-div numericfield numericformat longField" disabled="true">
    ...
    <div class="form-text_div pib_initial_cost">
        ...
        <#if isProjectOwner || isAboveMarketProjectContactUser || isAdminUser || isGlobalSuperUser >
		     ...
		     <#if isAboveMarket>
		        <#--<input type="text" name="name" value="${project.totalCost}" size="30" class="form-text-div numericfield numericformat longField" disabled="true">
...
<#if isAdminUser>
	...
	<label class='rte-editor-label'>Project Description</label>
		<div class="form-text_div">	
			<textarea id="description" name="description" rows="10" cols="20" class="form-text-div">${projectSpecsInitiation.description?default('')?html}</textarea>    
...
<div class="pib-report-requirement">
    ...
    <table class="report_requirement_list">
        ...
        <tr>
            <#if isAdminUser>
            	<td><@renderReportingCheckBox label='' name='globalSummary' isChecked=projectSpecsReporting.globalSummary?default(false) disable=false /></td>
            <#else>
        ...
        <div id="jive-file-list" class="jive-attachments">
            <#list attachmentMap.get(screenerID) as attachment>
                <div id="jive-attachment-${attachment.ID?c}" class="jive-attach-item">
                    <span class="jive-icon-med jive-icon-attachment"></span>
                     <#if (attachmentUser?? && attachmentUser.get(attachment.ID)?? && attachmentUser.get(attachment.ID)==user.ID) || (isAdminUser) >
                        <a href="javascript:void(0);" class="j-remove-file font-color-meta" onclick="javascript:removeAttachment(${attachment.ID?c}, ${screenerID.ID?c}, '${attachment.name}');"><@s.text name="global.remove" /></a>
                    </#if>
                    <a href="<@s.url value="/servlet/JiveServlet/download/${attachment.objectID?c}-${attachment.ID?c}/${attachment.name?url}" />" >
                    ${attachment.name?html} (${statics['com.jivesoftware.util.ByteFormat'].getInstance().format(attachment.size)})  </a>
                </div>
...
<div class="pib_angola_left">
    ...
    <p class="induction_text">If any of fields are not relevant, please put '0' in it.</p>
        ...
        <#if isAdminUser>
	     ...
	     <#elseif isCountryProjectContact || isCountrySPIContact || isAboveMarketProjectContactUser>
    ...
    <div id="internation_management_cost_div" style='display:${internation_management_cost_display};'>
        ...
        <div class="form-select_div">
	        ...
	        <#elseif isCountryProjectContact || isCountrySPIContact || isAboveMarketProjectContactUser>
	        	<input type="text" name="fieldworkCost-display" disabled onchange="fieldWorkCostChange()" <#if projectSpecsEMDetails.fieldworkCost??  > value="${projectSpecsEMDetails.fieldworkCost?default('')}" <#else> value="" </#if>  class="form-text-div numericfield numericformat longField" >
		...
		<div class="form-select_div">
	        ...
	        <#elseif isCountryProjectContact || isCountrySPIContact || isAboveMarketProjectContactUser>
	        	<input type="text" name="operationalHubCost-display" disabled onchange="operationalHubCostChange()" <#if projectSpecsEMDetails.operationalHubCost??  > value="${projectSpecsEMDetails.operationalHubCost}" <#else> value="" </#if> size="20" class="form-text-div numericfield numericformat longField" >
        ...
        <div class="form-select_div">
	        ...
	        <#elseif isCountryProjectContact || isCountrySPIContact || isAboveMarketProjectContactUser>
	        	<input type="text" name="otherCost-display" disabled onchange="otherCostChange()" <#if projectSpecsEMDetails.otherCost??  > value="${projectSpecsEMDetails.otherCost}" <#else> value="" </#if> size="20" class="form-text-div numericfield numericformat longField" >
...
<div id="emailNotification" style="display:none">
    <form id="email-notification-form" action="<@s.url value="/synchro/project-multi-specs!sendNotification.jspa"/>" method="post" enctype="multipart/form-data" class="j-form-popup">
        ...
        <input type="hidden" id="endMarketId" name="endMarketId" value="${endMarketId}" />
...
<form method="POST" name="moveToNextStageForm" id="moveToNextStageForm" action="/synchro/project-multi-specs!moveToNextStage.jspa"  >
...
<div id="initiateWaiver" style="display:none">
    ...
    <#if isAdminUser>
    	 ...
    	 <form id="waiver-form" action="<@s.url value="/synchro/project-multi-specs!updateWaiver.jspa"/>" method="post">
    ...
    <#else>
        ...
        <form id="waiver-form" action="<@s.url value="/synchro/project-multi-specs!updateWaiver.jspa"/>" method="post">
            ...
            <div class="pib_view_popup form-text_div">
                ...
                <#if (isMethAppUser || isProcUser || isProductUser) && !(isProjectOwner || isSPIContactUser || isProjectCreator)>
                    <textarea id="methodologyDeviationRationale" disabled name="methodologyDeviationRationale" placeholder="Methodology Description">${pibMethodologyWaiver.methodologyDeviationRationale?default('')?html}</textarea>
...
<#--<#if isAboveMarket> -->
	<div id="fieldWorkWindow" style="display:none">
		<div class="popup-waiver">
			...
			<div class="end_market_details_div" style="display:block;">
				<form id="fieldwork-form-popup" action="<@s.url value="/synchro/project-multi-specs!updateFielwork.jspa"/>" method="post" class="j-form-popup fieldwork-form-popup">
					...
					<div class="pro-planned-dates">
						<div class="pib_view_popup">
							<div class="form-date_div">
								...
								<label>Project End<br/>(Results)</label>
								<input type="text" name="cities" disabled value="<#if projectEndDate?? >${projectEndDate?string('dd/MM/yyyy')}</#if>" class="form-text-div" >
							</div>
					...
					<#if isProjectOwner || isAboveMarketProjectContactUser || isAdminUser >
						<div class="pib_view_popup pro-latest-field last">
									<label>Original Cost</label>
									<input type="text" name="originalfinalCost-display" value="${aboveMarketFinalCost?default('')}" disabled class="form-text-div numericfield numericformat longField" >
	                                <@renderCurrenciesField name='originalfinalCostType-oo'  value=aboveMarketFinalCostType?default(defaultCurrency) disabled=true/>
								</div>
	
								<div class="pib_view_popup pro-latest-field last">
									<label>Final Cost</label>
									<script type="text/javascript">
										function aboveMarketFinalCostChange() {
											var val = Number($j("input[name=aboveMarketFinalCost-display]").val().replace(/\,/g,''));
											if(val) {
												$j("input[name=aboveMarketFinalCost]").val(val);
											} else {
												$j("input[name=aboveMarketFinalCost]").val("");
											}
										}
									</script>
									<input type="text" name="aboveMarketFinalCost-display" onchange="aboveMarketFinalCostChange()" value="${aboveMarketFinalCost?default('')}" class="form-text-div numericfield numericformat longField" title="If the project has a fieldwork tendering process then the final cost does not include the fieldwork cost">
									<@renderCurrenciesField name='aboveMarketFinalCostType'  value=aboveMarketFinalCostType?default(defaultCurrency) disabled=false/>
									<@macroCurrencySelectError msg="Please select currency" />
									<@macroCustomFieldErrors msg='Please enter final cost' />
									<@macroCustomFieldErrors msg='' class='numeric-error' />
									<input type="hidden" name="aboveMarketFinalCost"  value="<#if aboveMarketFinalCost??>${aboveMarketFinalCost?c}</#if>" />
					...
					<#elseif isGlobalSuperUser >
						<div class="pib_view_popup pro-latest-field last">
								...
								<div class="pib_view_popup pro-latest-field last">
									...
									<input type="text" name="aboveMarketFinalCost-display" disabled onchange="aboveMarketFinalCostChange()" value="${aboveMarketFinalCost?default('')}" class="form-text-div numericfield numericformat longField" title="If the project has a fieldwork tendering process then the final cost does not include the fieldwork cost">
									<@renderCurrenciesField name='aboveMarketFinalCostType'  value=aboveMarketFinalCostType?default(defaultCurrency) disabled=true/>
									<@macroCurrencySelectError msg="Please select currency" />
									<@macroCustomFieldErrors msg='Please enter final cost' />
									<@macroCustomFieldErrors msg='' class='numeric-error' />
									<input type="hidden" name="aboveMarketFinalCost"  value="<#if aboveMarketFinalCost??>${aboveMarketFinalCost?c}</#if>" />
					...
					<#list psEMList as psEMDetail>
						<#assign emid = psEMDetail.endMarketID />
						 ...
						 <#assign isCountryProjectContactFW = statics['com.grail.synchro.util.SynchroPermHelper'].isCountryProjectContact(projectID,emid) />
						 <#assign isCountrySPIContactFW = statics['com.grail.synchro.util.SynchroPermHelper'].isCountrySPIContact(projectID,emid) />
						 <#assign isEndMarketSuperUser = statics['com.grail.synchro.util.SynchroPermHelper'].isSynchroEndmarketSuperUser(user,emid) />
						
							<h3 id="block_header_${emid}" onclick="javascript:toggleEndmarketBlock(${emid});">
								${statics['com.grail.synchro.SynchroGlobal'].getEndMarkets().get(emid?int)}
								<a id="legend_${emid}" href="javascript:void(0);" class="close-form"></a>
							</h3>
						<div id="endMarket_${emid}">
							<div class="pro-planned-dates">
							...
							<#if projectSpecsInitiation.methodologyType!=4 >
								<div class="pib_view_popup">
									...
									<div class="form-date_div">
										...
										<input type="text" name="cities" disabled value="<#if psEMDetail.fwStartDate?? >${psEMDetail.fwStartDate?string('dd/MM/yyyy')}</#if>" class="form-text-div" >
									</div>
									<div class="form-date_div right">
										<label>Fieldwork  End</label>
										<input type="text" name="cities"  disabled value="<#if psEMDetail.fwEndDate?? >${psEMDetail.fwEndDate?string('dd/MM/yyyy')}</#if>" class="form-text-div" >
									</div>
								</div>
								<div class="pib_view_popup">
									...
									<div class="form-date_div">
										<label>Fieldwork Begin</label>
										<#--<@jiveform.datetimepicker id="fwStartDateLatest_${emid}" name="fwStartDateLatest_${emid}" value="" readonly=true disablePrevDates=false defaultDateTimePicker=false/>-->
										<@synchrodatetimepicker id="fwStartDateLatest_${emid}" name="fwStartDateLatest_${emid}" value="" format="%d/%m/%Y" readonly=true disablePrevDates=false defaultDateTimePicker=false/>
										<@macroCustomFieldErrors msg='' />
									</div>
									<div class="form-date_div right">
										<label>Fieldwork  End</label>
										<#--<@jiveform.datetimepicker id="fwEndDateLatest_${emid}" name="fwEndDateLatest_${emid}" value="" readonly=true disablePrevDates=false defaultDateTimePicker=false/>-->
											<@synchrodatetimepicker id="fwEndDateLatest_${emid}" name="fwEndDateLatest_${emid}" value="" format="%d/%m/%Y" readonly=true disablePrevDates=false defaultDateTimePicker=false/>
										<@macroCustomFieldErrors msg='' />
									</div>
								</div>
							</#if>
							...
							<div class="pib_view_popup pro-latest-field">								
								<textarea id="latestFWComments_${emid}" name="latestFWComments_${emid}">${psEMDetail.latestFWComments?default('')}</textarea>                                
							</div>

							<#if isCountryProjectContactFW || isCountrySPIContactFW || isAdminUser >
								<div class="pib_view_popup pro-latest-field last">
									<label>Original Cost</label>
									<input type="text" name="originalfinalCost-display" value="${psEMDetail.originalFinalCost?default('')}" disabled class="form-text-div numericfield numericformat longField" >
	                                <@renderCurrenciesField name='originalfinalCostType_${emid}'  value=psEMDetail.originalFinalCostType?default(defaultCurrency) disabled=true/>
								</div>
	
								<div class="pib_view_popup pro-latest-field last">
									<label>Final Cost</label>
									<script type="text/javascript">
										function finalCostChange(eId) {
											var val = Number($j("input[name=finalCost-display_"+eId+"]").val().replace(/\,/g,''));
											if(val) {
												$j("input[name=finalCost_"+eId+"]").val(val);
											} else {
												$j("input[name=finalCost_"+eId+"]").val("");
											}
										}
									</script>
									<input type="text" name="finalCost-display_${emid}" onchange="finalCostChange('${emid}')" value="${psEMDetail.finalCost?default('')}" class="form-text-div numericfield numericformat longField" title="If the project has a fieldwork tendering process then the final cost does not include the fieldwork cost">
									<@renderCurrenciesField name='finalCostType_${emid}'  value=psEMDetail.finalCostType?default(defaultCurrency) disabled=false/>
									<@macroCurrencySelectError msg="Please select currency" />
									<@macroCustomFieldErrors msg='Please enter final cost' />
									<@macroCustomFieldErrors msg='' class='numeric-error' />
									<input type="hidden" name="finalCost_${emid}"  value="<#if psEMDetail.finalCost??>${psEMDetail.finalCost?c}</#if>" />
								</div>
							<#elseif isAboveMarketProjectContactUser || isProjectOwner || isGlobalSuperUser || isEndMarketSuperUser>
								
								<div class="pib_view_popup pro-latest-field last">
									<label>Original Cost</label>
									<input type="text" name="originalfinalCost-display" value="${psEMDetail.originalFinalCost?default('')}" disabled class="form-text-div numericfield numericformat longField" >
	                                <@renderCurrenciesField name='originalfinalCostType'  value=psEMDetail.originalFinalCostType?default(defaultCurrency) disabled=true/>
								</div>
								...
								<div class="pib_view_popup pro-latest-field last">
									...
									<input type="text" name="finalCost-display_${emid}" disabled onchange="finalCostChange('${emid}')" value="${psEMDetail.finalCost?default('')}" class="form-text-div numericfield numericformat longField" title="If the project has a fieldwork tendering process then the final cost does not include the fieldwork cost">
									<@renderCurrenciesField name='finalCostType_${emid}'  value=psEMDetail.finalCostType?default(defaultCurrency) disabled=true/>
									<@macroCurrencySelectError msg="Please select currency" />
									<@macroCustomFieldErrors msg='Please enter final cost' />
									<@macroCustomFieldErrors msg='' class='numeric-error' />
									<input type="hidden" name="finalCost_${emid}"  value="<#if psEMDetail.finalCost??>${psEMDetail.finalCost?c}</#if>" />
							...
							<#if isCountryProjectContactFW || isCountrySPIContactFW || isAdminUser >
								...
								<div class="pib_view_popup pro-latest-field last">								
									<textarea id="finalCostComments_${emid}" name="finalCostComments_${emid}">${psEMDetail.finalCostComments?default('')}</textarea>
					...
					<div>
						<input class="j-btn-callout" type="submit" name="doPost"  id="fieldwork-form-popup-btn"  value="Save" style="font-weight: bold;" />				
						<input type="hidden" name="projectID" value="${projectID?c}">
						<input type="hidden" name="endMarketId" value="${endMarketId?c}">
...
<!-- ADD/CHANGE COUNTRY WINDOW STARTS -->
	...
	<div id="addChangeCountryWindow" style="display:none">
	    ...
	    <form id="update-end-markets-form" action="<@s.url value="/synchro/project-multi-specs!updateEndMarkets.jspa"/>" method="post">
	        <div class="form-select_div_main">
	            <!-- End Markets Multiselect -->
	        <@showUpdatedEndMarketFieldSection name='updatedEndMarkets' value=emIds?default('-1') label='End Markets' validate=false />
	        <#assign error_msg><@s.text name='project.error.endmarket' /></#assign>
	        <@macroCustomFieldErrors msg=error_msg />
	        </div>
...
<#list psEMList as psEMDetail>
    <#assign emid = psEMDetail.endMarketID />
var _fwStartDateLatest = <#if psEMDetail.fwStartDateLatest?? >"${psEMDetail.fwStartDateLatest?string('dd/MM/yyyy')}";<#else>"";</#if>
var _fwEndDateLatest = <#if psEMDetail.fwEndDateLatest?? >"${psEMDetail.fwEndDateLatest?string('dd/MM/yyyy')}";<#else>"";</#if>

    $j("#fwStartDateLatest_${emid}").val(_fwStartDateLatest);
    $j("#fwEndDateLatest_${emid}").val(_fwEndDateLatest);
   

</#list>
var _projectEndDateLatest = <#if projectEndDateLatest?? >"${projectEndDateLatest?string('dd/MM/yyyy')}";<#else>"";</#if>
...
<#if showMandatoryFieldsError?? && showMandatoryFieldsError>
    ...
    $j( "#fieldwork-form-popup-btn" ).click(function() {
		showLoader('Please wait...');
        $j( ".jive-error-message" ).each(function( index ) {
            if(!$j(this).hasClass('numeric-error'))
            {
    ...
    <#list psEMList as psEMDetail>
        ...
        <#if projectSpecsInitiation.methodologyType!=4 >
            var fwStartDateLatest = $j("input[name=fwStartDateLatest_${emid}]");
            ...
            var fwEndDateLatest = $j("input[name=fwEndDateLatest_${emid}]");
        ...
        var finalCost = $j("input[name=finalCost-display_${emid}]");
        var finalCostVal = $j("input[name=finalCost-display_${emid}]").val().replace(/\,/g, '');
        var finalCostVal_prev = "<#if psEMDetail.finalCost??>${psEMDetail.finalCost?c}<#else></#if>";
        ...
        if(finalCostVal!=finalCostVal_prev && !error)
        {
            ...
            if(!error)
            {                
				var finalCostComments = $j("textarea[name=finalCostComments_${emid}]");
        ...
        if(!error)
        {
            $j("input[name=finalCost_${emid}]").val($j("input[name=finalCost-display_${emid}]").val().replace(/\,/g, ''));
        }
		else
		{
			 hideLoader();
...
<!-- Audit Logs -->
	...
	var projName = "${project.name?js_string}";
	SynchroLogUtil.addLogActivity("${statics['com.grail.synchro.SynchroGlobal$PortalType'].SYNCHRO.getDescription()}", ${statics['com.grail.synchro.SynchroGlobal$PageType'].PROJECT.getId()}, ${statics['com.grail.synchro.SynchroGlobal$Activity'].VIEW.getId()}, ${statics['com.grail.synchro.SynchroGlobal$LogProjectStage'].PROJECT_SPECS.getId()}, "${i18CustomText}", projName, ${projectID?c}, ${user.ID?c}, ${endMarketId?c});
</script>