...
<div class="container-sub">
    <div class="project_view">
        <div class="project_dashboard_header">          
            <h2><#if projectID?? && (projectID>0)>Project Logs<#else>All Logs</#if></h2><br>
			<#if projectName??><h2>${projectName?html} (${generateProjectCode('${projectID?c}')}) </h2></#if>
...
<#if projectID?? && (projectID>0)>
	<form id="generate-log-form" style="display:none;" action="<@s.url value="/synchro/dashboard-log-pagination!downloadProjectLogFile.jspa"/>" method="post"></form>
<#else>
	<form id="generate-log-form" style="display:none;" action="<@s.url value="/synchro/dashboard-log-pagination!downloadLogFile.jspa"/>" method="post"></form>
</#if>
...
<script type="text/javascript">
    ...
    $j(document).ready(function () {
		...
		<#if projectID?? && (projectID>0)>
		PROJECT_STAGE_NAVIGATOR.initialize({projectId:${projectID?c}, activeStage:-1});
		</#if>
        currPage = 1;
...
//        $j("#ascendingOrder").val("");
    ...
    function processPaginationRequest(page,keyword)
    {
        ...
        var url = '/synchro/dashboard-log-pagination.jspa'
        if(xhr != null)
            xhr.abort();
        ...
        xhr = $j.ajax({
            url: url,
            type: 'POST',
            data: filterForm.serialize() + '&keyword='+ keyword+"&page="+page+"&projectID="+projectID,
            beforeSend: function() {
                // $j("#overlay").show();
            },
            complete: function() {
