...
function confirmDelete(attachmentId, fieldID, attachmentName) {
    location.href="/new-synchro/project-eval!removeAttachment.jspa?projectID=${projectID?c}&attachmentId="+attachmentId+"&attachmentFieldID="+fieldID+"&attachmentName="+attachmentName;
}
...
<h2 class="project_eval_hyperlink">${project.name?html} (${generateProjectCode('${projectID?c}')}) </h2>
<a class="view-edit closeonloadPopup" href="javascript:void(0);" onclick="javascript:openPITWindow()">Click here to view more details</a>
...
<#if !canEditStage>	
	<#include "/template/global/project-evaluation-new-readOnly.ftl" />
<#else>
		...
		<div class="boxinnerContainer">
			...
			<#if projectEvalCounter gt 0>
					...
					<!--<input type="text" placeholder="enter your text" onfocus="this.placeholder = ''" onblur="this.placeholder = 'enter your text'" />  -->
						<textarea id="comment" name="comments"  rows="5" cols="20" placeholder="Comments"  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Comments'" class="form-text-div">${projEvaluation.getAgencyComments()}</textarea>
					<#else>	
						<textarea id="comment" name="comments"  rows="5" cols="20" placeholder="Comments"  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Comments'" class="form-text-div"></textarea>
					</#if>
				<#else>
					<#if projEvaluation.getAgencyComments()?? >
						<textarea id="comment" name="comment"  rows="5" cols="20" placeholder="Comments"   onfocus="this.placeholder = ''" onblur="this.placeholder = 'Comments'"   class="form-text-div">${projEvaluation.getAgencyComments()}</textarea>
					<#else>
...
<script type="text/javascript">
    ...
    $j(document).ready(function(){
        attachmentWindow = new SYNCHRO_FIELD_ATTACHMENT_POPUP({
            formUrl:"<@s.url value='/new-synchro/project-eval!addAttachment.jspa'/>",
            projectID:${projectID?c},
...
<#-- ELSE BLOCK ENDS in case the of Read only access -->
...
<div id="initiateWaiver" style="display:none">
    ...
    <div class="">
        ...
        <form id="waiver-form" action="<@s.url value='/new-synchro/project-eval!updateWaiver.jspa'/>" method="post" class="j-form-popup">
...
<div id="initiateKantarWaiver" class="j-form-popup"  style="display:none">
    ...
    <div class="">
        ...
        <form id="kantar-waiver-form" action="<@s.url value='/new-synchro/project-eval!updateKantarWaiver.jspa'/>" method="post">
		<div class="popup-title-overlay"></div>
...
<div id="pitWindow" style="display:none">
    <div class="view_edit_pib">
        ...
        <form id="synchro-pit-form" action="<@s.url value='/new-synchro/project-eval!updatePIT.jspa'/>" method="post" class="j-form-popup">
	...
	<!--amit-->
			...
			<div class="region-inner">
				...
				<div class="region-inner">
					...
					<span class="jive-error-message full-width" id="methodologyWaiverError" style="display:none">Please select methodology waiver be required</span>
					</div>
...
<form method="POST" name="cancelProj" id="cancelProj" action="/new-synchro/project-eval!cancelProject.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
</form>

<form method="POST" name="enableProj" id="enableProj" action="/new-synchro/project-eval!enableProject.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
</form>

<form method="POST" name="resetRSForm" id="resetRSForm" action="/new-synchro/project-eval!resetReportSummary.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
</form>
...
$( document ).on( "click", ".removeRS", function() {
  ...
  function resetRS()
  {
	dialog({
			title:"",
			html:"<i class='warningErr positionSet'></i><p>Are you sure you want to reset the Project?</p>",
			buttons:{
			"YES":function() {
				var resetRSForm = jQuery("#resetRSForm");
				resetRSForm.submit();
			},
			"NO": function() {
				return false;
