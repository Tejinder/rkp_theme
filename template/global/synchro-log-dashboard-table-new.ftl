...
<div class="project_dashboard_table_left">
    <table id="project_logs_table_body" class="scrollable-table project_status_table tablesorter">
		...
		<th class="log-description activitydesWidth"><span>Activity Description</span></th>
		<th class="log-projectcode sortable projectcodeWidth"><span id="id">Project Code</span></th>
		<th class="log-projectname sortable pnameWidth"><span id="name">Project Name</span></th>
		<th class="log-projectname sortable waiverIdWidth"><span id="waiverId">WaiverId</span></th>
        </thead>
        <tbody>
            <#list logs as log>		
			...
			<#list log.descriptions as description>						
				<tr id="log-${log_index}-${description_index}" <#if !showRow>style="display:none;"</#if><#if (log_index % 2) == 0> class="last"</#if>>
					...
					<td class="log-description activitydesWidth">
					...
					<td class="log-projectcode projectcodeWidth">
						<span><#if description_index==0><#if log.projectID??>${log.projectID?c}</#if></#if></span>
					</td>
					<td class="log-projectname pnameWidth">
						<span><#if description_index==0>${log.projectName}</#if></span>
					</td>
					<td class="log-projectname waiverIdWidth">
						<span><#if description_index==0><#if log.waiverID??>${log.waiverID?c}</#if></#if></span>
					</td>
				</tr>
