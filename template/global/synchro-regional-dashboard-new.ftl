...
//        $j("#ascendingOrder").val("");
    ...
    {
        currPage = page;
        var url = '/new-synchro/regional-dashboard-pagination.jspa'
        if(xhr != null)
            xhr.abort();
...
function processDownloadRequest(page, keyword, plimit, downloadReportType) {
	...
	$j("#result-filter-form").prop("action", "/new-synchro/regional-dashboard-pagination!downloadReport.jspa");
	var filterForm = $j("#result-filter-form");
...
<script type="text/javascript">
<!-- Audit Logs -->
	<#assign i18CustomText>Regional All Projects</#assign>
