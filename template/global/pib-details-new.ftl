...
<div class="region-inner">
	<a  href='${JiveGlobals.getJiveProperty("synchro.brief.template.path"," /file/download/AgencyCostDetails-SingleMarketTemplate.xlsx")}'>Click here to download Project Brief template</a>
...
<#if project.processType?? && (project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].END_MARKET_EU_ONLINE.getId() || project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_EU_ONLINE.getId())>
	...
	<div class="project_owner_names legal_approval_dropdown_height">
			...
			<#if projectInitiation?? && projectInitiation.sendForApproval?? && projectInitiation.sendForApproval==1 >
				<select data-placeholder="Select Legal Approver's Name" disabled class="chosen-select" id = "briefLegalApprover" name='briefLegalApprover' >
		...
		<#if projectInitiation?? && projectInitiation.sendForApproval?? && projectInitiation.sendForApproval==1 >
			...
			<input type="button" name=""  id="sendReminder" onclick="updateSendReminder();" value="Send Reminder" class="save approval-btn btn-blue border-inputradius"/>
			...
			<span class="approval-status   pendingStatus">TPD Status: Pending</span>
...
<span class="jive-error-message full-width" id="briefLegalApproverError" style="display:none">Please select Legal Approver’s Name</span>
	 </div>    
	...
	<#else>
	 <div class="project_owner_names legal_approval_dropdown_height">
			...
			<#elseif projectInitiation?? && projectInitiation.legalApprovalStatus?? && (projectInitiation.legalApprovalStatus == 1 || projectInitiation.legalApprovalStatus == 2)  >
				<select data-placeholder="Select Legal Approver's Name" disabled class="chosen-select" id = "briefLegalApprover" name='briefLegalApprover' >
			<#else>
				<select data-placeholder="Select Legal Approver's Name"  class="chosen-select" id = "briefLegalApprover" name='briefLegalApprover' >
		...
		<#if projectInitiation?? && projectInitiation.sendForApproval?? && projectInitiation.sendForApproval==1 >
			<div class="reminder-wrapper">
			<input type="button"   id="sendReminder" name=""  onclick="updateSendReminder();" value="Send Reminder" class="save approval-btn btn-blue border-inputradius"/>
			<#if projectInitiation?? && projectInitiation.sendReminderDate??>
			<p>Reminder Sent On ${projectInitiation.sendReminderDate?string('dd/MM/yyyy')}</p>
			</#if>
			</div>
			<span class="approval-status">TPD Status: <sapn class="status-txt pendingStatus">Pending</span></span>
		<#elseif projectInitiation?? && projectInitiation.legalApprovalStatus?? && (projectInitiation.legalApprovalStatus == 1)  >
			<span class="approval-status">TPD Status: <sapn class="status-txt">May have to be TPD Submitted</span></span>
		<#elseif projectInitiation?? && projectInitiation.legalApprovalStatus?? && (projectInitiation.legalApprovalStatus==2)  >
			<span class="approval-status">TPD Status: <sapn class="status-txt">Does not have to be TPD Submitted</span></span>
		<#else>
			<#--<#if projectInitiation?? && projectInitiation.briefLegalApprover?? && projectInitiation.briefLegalApprover gt 0>
				<input type="button" name="" onclick="updateSendForApproval();" value="Send For Approval" class="save approval-btn btn-blue"/>
	   ...
	   <#if projectInitiation?? && projectInitiation.legalSignOffRequired?? && projectInitiation.legalSignOffRequired==1>
		<div class="project_status_div">
		   ...
		   <div class="status_right_group">
				...
				<input type="button" name="" id="adminConfirmSubmisssion" onclick="confirmLegalApprovalSubmission();" value="Approved for project to proceed" class="save confirm 	btn-yellow disabled"   />
				<span>OR</span>
				<input type="button" name=""  id="adminUpdateSubmisssion" onclick="updateNeedDiscussion();" value="Needs Discussion" class="save need-discussion btn-blue disabled" />
				</div>
				</div>
		   <#else>
		        <div class="project_status_div">
	   ...
	   <!--amit code-->
	    <div class="project_owner_names legal_approval_dropdown_height">
				...
				<#--<#if isSynchroSystemOwner>
					...
					<select data-placeholder="Enter Legal Approver’s Name" disabled class="chosen-select" id = "briefLegalApprover" name='briefLegalApprover' >
				<#elseif projectInitiation?? && projectInitiation.sendForApproval?? && projectInitiation.sendForApproval==1 >
					<select data-placeholder="Enter Legal Approver’s Name" disabled class="chosen-select" id = "briefLegalApprover" name='briefLegalApprover' >
				<#else>
					<select data-placeholder="Enter Legal Approver’s Name" disabled class="chosen-select" id = "briefLegalApprover" name='briefLegalApprover' >
				...
				<#if isSynchroSystemOwner>
					<#if projectInitiation.getBriefLegalApproverOffline()??>
						<input type="text" id="briefLegalApprover" placeholder="Enter Legal Approver’s Name" name="briefLegalApproverOffline" disabled  value="${projectInitiation.getBriefLegalApproverOffline()}"  />
					<#else>
						<input type="text" id="briefLegalApprover" placeholder="Enter Legal Approver’s Name" name="briefLegalApproverOffline"  disabled value=""  />
					</#if>	
				<#elseif projectInitiation?? && projectInitiation.sendForApproval?? && projectInitiation.sendForApproval==1 >
					<#if projectInitiation.getBriefLegalApproverOffline()??>
						<input type="text" id="briefLegalApprover" placeholder="Enter Legal Approver’s Name" name="briefLegalApproverOffline"  disabled value="${projectInitiation.getBriefLegalApproverOffline()}"  />
				...
				<#if attachmentMap?? && attachmentMap.get(briefLegalApprovalID)?? >
					<div id="jive-file-list" class="attachmentsNew attachmentsNew__fix	 jive-attachments">
				 ...
				 <span class="full-width jive-error-message" id="briefLegalApproverError" style="display:none">Please enter Legal Approver’s Name</span>
		...
		<#if isSynchroSystemOwner>
				...
				<#else>
					<@synchrodatetimepicker id="legalApprovalDate" name="legalApprovalDate" value="" format="%d/%m/%Y" readonly=true disablePrevDates=false defaultDateTimePicker=false placeholder='Enter Legal Approval Date'/>
				</#if>	
			<#else>
	...
	<#else>
		<#if !isBriefLegalApprover>
		 <div class="project_owner_names legal_approval_dropdown_height">
				...
				<#--<#if isSynchroSystemOwner>
					<select data-placeholder="Enter Legal Approver’s Name"  class="chosen-select" id = "briefLegalApprover" name='briefLegalApprover' >
				<#elseif projectInitiation?? && projectInitiation.sendForApproval?? && projectInitiation.sendForApproval==1 >
					<select data-placeholder="Enter Legal Approver’s Name" disabled class="chosen-select" id = "briefLegalApprover" name='briefLegalApprover' >
				<#else>
					<select data-placeholder="Enter Legal Approver’s Name"  class="chosen-select" id = "briefLegalApprover" name='briefLegalApprover' >
				...
				<#if isSynchroSystemOwner>
					<#if projectInitiation.getBriefLegalApproverOffline()??>
						<input type="text" id="briefLegalApprover" placeholder="Enter Legal Approver’s Name" name="briefLegalApproverOffline"  value="${projectInitiation.getBriefLegalApproverOffline()}"  />
					<#else>
						<input type="text" id="briefLegalApprover" placeholder="Enter Legal Approver’s Name" name="briefLegalApproverOffline"  value="" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"  />
				...
				<#elseif projectInitiation?? && projectInitiation.sendForApproval?? && projectInitiation.sendForApproval==1 >
					...
					<#if projectInitiation.getBriefLegalApproverOffline()??>
						<input type="text"  id="briefLegalApprover" placeholder="Enter Legal Approver’s Name" name="briefLegalApproverOffline" disabled value="${projectInitiation.getBriefLegalApproverOffline()}"   onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"/>
					<#else>
						<input type="text"  id="briefLegalApprover" placeholder="Enter Legal Approver’s Name" name="briefLegalApproverOffline" disabled value=""  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'" />
					</#if>	
				<#else>
					<#if projectInitiation.getBriefLegalApproverOffline()??>
						<input type="text"  id="briefLegalApprover" placeholder="Enter Legal Approver’s Name" name="briefLegalApproverOffline"  value="${projectInitiation.getBriefLegalApproverOffline()}"  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'" />
					<#else>
						<input type="text"  id="briefLegalApprover" placeholder="Enter Legal Approver’s Name" name="briefLegalApproverOffline"  value=""  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"/>
					</#if>	
				</#if>
	...
	<!--amit-->
			...
			<div class="region-inner">
					...
					<div class="col-lg-6">
						...
						<#else>
							<select title=""  data-placeholder="Select Methodology" class="chosen-select" id = "methodologyDetails" name = "methodologyDetails" >
						</#if>
						  <option value=""></option>
			...
			<div class="region-inner two-column">
				...
				<div class="form-select_div">
					...
					<#if project.endMarketFunding??>
                          ...
                          <span class='jive-error-message endmarketfunding_alignment' id='endMarketFundingErrors' style='display: none;'>Please select is an end market involved in this project? </span>
					<#else>
				...
				<div class="pitfundMsg__fix">
					<#if project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_NON_EU.getId()>
						...
						</select>	 <span class='jive-error-message fundingMarketsError pop_fundErr__fix'  style='display: none;'> Please select the end market(s) which will carry out the fieldwork</span>
					...
					<#else>
						...
						</select>	 <span class='jive-error-message fundingMarketsError pop_fundErr__fix'  style='display: none;'>Please select the end market which will carry out the fieldwork</span>
...
//Amit'code
function validatePIBFormFields1(continueVar,processType)
{
	...
	if(processType=="offLine")
	  {
		...
		{		
			...
			if(briefLegalApprover.val() != null && $j.trim(briefLegalApprover.val())=="")
			{
			     ...
			     var isDisabled = $j("#briefLegalApprover").prop("disabled");
 ...
 if(processType=="onLine")
	  {
		...
		{		
			...
			var isDisabledbriefLegalApprover = $j("#briefLegalApprover").prop("disabled");
...
<script type="text/javascript">
  ...
  function continueProject()
  {
			...
			else
			{
				...
				var error = validatePIBFormFields1("continue",$j("#processType").val());
	...
	$j(document).ready(function() {
	 ...
	 $j("#legalsignoffreq").change(function() {
			...
			{	
				...
				$j("#adminConfirmSubmisssion").addClass("disabled");
				$j("#adminUpdateSubmisssion").addClass("disabled");
			...
			//	$j('#confirmProjectEnable').addClass("disabled");
				...
				$j("#adminConfirmSubmisssion").removeClass("disabled");
				
				$j("#adminUpdateSubmisssion").removeClass("disabled");
		...
		if(startDate!=null && startDate!=undefined && startDate!="")
		{
			if(!compareDate(startDate, endDate))
			{
			 ...
			 dialog({
					...
					"OK":function() {
						$j("#endDate1").val('');
                        //$j("#endDate1").parent().parent().find("span").show();
						return false;		
					}
				},
