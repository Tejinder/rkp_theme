...
//        $j("#ascendingOrder").val("");
    ...
    function processPaginationRequest(page,keyword, plimit)
    {
		...
		currPage = page;
        var url = '/new-synchro/em-dashboard-close-pagination.jspa'
        if(xhr != null)
            xhr.abort();
...
$j("#budgetYearSelected").val(bYear);
	$j("#result-filter-form").prop("action", "/new-synchro/em-dashboard-close-pagination!downloadReport.jspa");
	var filterForm = $j("#result-filter-form");
...
<script type="text/javascript">
<!-- Audit Logs -->
	<#assign i18CustomText>End-market Close Projects</#assign>
