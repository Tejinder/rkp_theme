...
<#assign isProposalLegalApprover = statics['com.grail.synchro.util.SynchroPermHelper'].isProposalLegalUser(projectID) />
...
<#--	FORM_DATA_PERSIST_HANDLER.handle({parentForm:$j("#proposal-form"),form:$j("#synchro-pit-form"), projectID:${projectID?c}}) -->
...
function removeAttachmentMessage(attachmentField) {
    dialog({
        title: '',
        html: '<p>Please upload a new '+attachmentField+' before removing the existing one </p>',
        buttons:{
            "OK":function() {
               closeDialog();
            }
...
function confirmDelete(attachmentId, fieldID, attachmentName) {
    location.href="/new-synchro/proposal-details!removeAttachment.jspa?projectID=${projectID?c}&attachmentId="+attachmentId+"&attachmentFieldID="+fieldID+"&attachmentName="+attachmentName;
}
...
<#if isProposalLegalApprover||isLegalApprover>  
...
<#if isProposalLegalApprover>
	<#include "/template/global/proposal-details-new-legalView.ftl" />
<#elseif !canEditStage>	
	<#include "/template/global/proposal-details-new-readOnly.ftl" />
<#else>
...
<#if project.processType?? && (project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].END_MARKET_EU_ONLINE.getId() || project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_EU_ONLINE.getId())>
   ...
   <input type="hidden"   id="processType"  value="onLine"/>
	<#if !isProposalLegalApprover>
	...
	<div class="project_owner_names legal_approval_dropdown_height">
			...
			<#if proposalInitiation?? && proposalInitiation.sendForApproval?? && proposalInitiation.sendForApproval==1 >
				<select data-placeholder="Select Legal Approver's Name" disabled class="chosen-select" id = "proposalLegalApprover" name='proposalLegalApprover' >
		...
		<#if proposalInitiation?? && proposalInitiation.sendForApproval?? && proposalInitiation.sendForApproval==1 >
			...
			<input type="button" name=""  id="sendReminder" onclick="updateSendReminder();" value="Send Reminder" class="save approval-btn btn-blue"/>
			...
			<span class="approval-status popTpd">TPD Status: <sapn class="status-txt  pendingStatus">Pending</span></span>
		<#elseif proposalInitiation?? && proposalInitiation.legalApprovalStatus?? && (proposalInitiation.legalApprovalStatus == 1)  >
			<span class="approval-status popTpd">TPD Status: <sapn class="status-txt">May have to be TPD Submitted</span></span>
		<#elseif proposalInitiation?? && proposalInitiation.legalApprovalStatus?? && (proposalInitiation.legalApprovalStatus==2)  >
			<span class="approval-status popTpd">TPD Status: <sapn class="status-txt">Does not have to be TPD Submitted</span></span>
		<#else>
			...
			<a href="javascript:void(0);" id="sendForApprovalEnable" style="margin-left: 10px;" onclick="return updateSendForApproval();" class="publish-details">Send For TPD Approval</a>	
	...
	<#else>
	 ...
	 <div class="project_owner_names legal_approval_dropdown_height">
			...
			<#elseif proposalInitiation?? && proposalInitiation.legalApprovalStatus?? && (proposalInitiation.legalApprovalStatus == 1 || proposalInitiation.legalApprovalStatus==2) >
				<select data-placeholder="Select Legal Approver's Name" disabled class="chosen-select" id = "proposalLegalApprover" name='proposalLegalApprover' >
			<#else>
				<select data-placeholder="Select Legal Approver's Name"  class="chosen-select" id = "proposalLegalApprover" name='proposalLegalApprover' >
			</#if>
			  <option value=""></option>
			...
			<input type="button" name="" id="sendReminder" onclick="updateSendReminder();" value="Send Reminder" class="save approval-btn btn-blue"/>
			<#if proposalInitiation?? && proposalInitiation.sendReminderDate??>
			<p>Reminder Sent On ${proposalInitiation.sendReminderDate?string('dd/MM/yyyy')}</p>
			</#if>
			</div>
			<span class="approval-status popTpd">TPD Status: <span class="status-txt pendingStatus">Pending</span></span>
		<#elseif proposalInitiation?? && proposalInitiation.legalApprovalStatus?? && (proposalInitiation.legalApprovalStatus == 1)  >
		...
		<#else>
			<#--<#if proposalInitiation?? &&  proposalInitiation.proposalLegalApprover?? &&  proposalInitiation.proposalLegalApprover gt 0>
				<input type="button" name="" onclick="updateSendForApproval();" value="Send For Approval" class="save approval-btn btn-blue"/>
			<#else>
				<input type="button" name="" value="Send For Approval"  class="save approval-btn btn-blue disabled"/>
			</#if>-->
	...
	<#if isProposalLegalApprover || isAdminUser || isSynchroSystemOwner >
		<div class="project_status_div">
		   ...
		   <input type="button" name="" id="adminConfirmSubmisssion" onclick="confirmLegalApprovalSubmission();" value="Approved for project to proceed" class="save confirm btn-yellow border-inputradius Approved_for_project_to_proceed_btn"/>
		   <span>OR</span>
...
<#if project.processType?? && (project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].END_MARKET_EU_OFFLINE.getId() || project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_EU_OFFLINE.getId())>
	...
	<input type="hidden"   id="processType"  value="offLine"/>
	<#if !isProposalLegalApprover>
	...
	<#if proposalInitiation?? && proposalInitiation.legalSignOffRequired?? && proposalInitiation.legalSignOffRequired==1>	
	 ...
	 <!--amit code-->
		...
		<#--	<#if isSynchroSystemOwner>
			...
			<#if isSynchroSystemOwner>
				<#if proposalInitiation.getProposalLegalApproverOffline()??>
					<input type="text" id="proposalLegalApprover" placeholder="Enter Legal Approver’s Name" name="proposalLegalApproverOffline"  disabled value="${proposalInitiation.getProposalLegalApproverOffline()}"   onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"/>
				<#else>
					<input type="text" id="proposalLegalApprover" placeholder="Enter Legal Approver’s Name" name="proposalLegalApproverOffline" disabled value=""  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"/>
				</#if>					
			<#elseif proposalInitiation?? && proposalInitiation.sendForApproval?? && proposalInitiation.sendForApproval==1 >
				<#if proposalInitiation.getProposalLegalApproverOffline()??>
					<input type="text" id="proposalLegalApprover" placeholder="Enter Legal Approver’s Name" name="proposalLegalApproverOffline"  disabled value="${proposalInitiation.getProposalLegalApproverOffline()}"  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"/>
			...
			<#if attachmentMap?? && attachmentMap.get(proposalLegalApprovalID)?? >
				<div id="jive-file-list" class="attachmentsNew jive-attachments  attachmentsNew__fix">
		...
		<#else>
			<div class="date-approval"><label>Date of Legal Approval</label> <@synchrodatetimepicker id="legalApprovalDate" name="legalApprovalDate" value="" format="%d/%m/%Y" readonly=true disablePrevDates=false defaultDateTimePicker=false dateCompareProposalLegal=true placeholder='Enter Legal Approval Date'/>
		</#if>	
	...
	<#else>
	 <div class="project_owner_names legal_approval_dropdown_height">
		...
		<#--	<#if isSynchroSystemOwner>
			...
			<#if isSynchroSystemOwner>
				<#if proposalInitiation.getProposalLegalApproverOffline()??>
					<input type="text" id="proposalLegalApprover" placeholder="Enter Legal Approver’s Name" name="proposalLegalApproverOffline"  value="${proposalInitiation.getProposalLegalApproverOffline()}"  />
				<#else>
					<input type="text" id="proposalLegalApprover" placeholder="Enter Legal Approver’s Name" name="proposalLegalApproverOffline"  value=""  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"/>
				</#if>	
			<#elseif proposalInitiation?? && proposalInitiation.sendForApproval?? && proposalInitiation.sendForApproval==1 >
			...
			<#else>
				<#if proposalInitiation.getProposalLegalApproverOffline()??>
					<input type="text" id="proposalLegalApprover" placeholder="Enter Legal Approver’s Name" name="proposalLegalApproverOffline"  value="${proposalInitiation.getProposalLegalApproverOffline()}"  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"/>
				<#else>
					<input type="text" id="proposalLegalApprover" placeholder="Enter Legal Approver’s Name" name="proposalLegalApproverOffline"  value=""  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"/>
				</#if>	
			</#if>
...
<#if !isProposalLegalApprover>
	 ...
	 <#if methBriefException == "yes">
		...
		<#if (project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].END_MARKET_EU_OFFLINE.getId() || project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_EU_OFFLINE.getId())>
			<div class="legal-checkbox" id="legal-checkbox"><input type="checkbox" name="legalsignoffreq" id="legalsignoffreq" <#if proposalInitiation?? && proposalInitiation.legalSignOffRequired?? && proposalInitiation.legalSignOffRequired==1> checked="true"</#if> >This project does not require legal sign-off on Proposal.<br></div>
		<#elseif proposalInitiation?? && proposalInitiation.sendForApproval?? && proposalInitiation.sendForApproval==1 >
...
<div class="buttons legalappr_btmmrgin">
    <#if !isProposalLegalApprover  >
        <br>
		<#if (isAdminUser || isSynchroSystemOwner) && proposalInitiation?? && proposalInitiation.legalApprovalStatus?? && proposalInitiation.legalApprovalStatus gt 0>
			<a href="javascript:void(0);" style="margin-left: 10px;" onclick="return resetProposal();" class="publish-details reset-btn">Reset Proposal</a>	
		<#elseif (isAdminUser || isSynchroSystemOwner) && proposalInitiation?? && proposalInitiation.sendForApproval?? && proposalInitiation.sendForApproval==1 >
			<a href="javascript:void(0);" style="margin-left: 10px;" onclick="return resetProposal();" class="publish-details reset-btn">Reset Proposal</a>	
		</#if>
		...
		<a href="javascript:void(0);" id="confirmProjectEnable" style="margin-left: 10px;" onclick="return continueProject();" class="publish-details">Confirm Project</a>	
    </#if>
	...
	<script>
		$j(document).ready(function() {
			...
			{	
				...
				$j('#proposalLegalApprover').prop('disabled', true).trigger('chosen:updated');
				
				$j('#legalApprovalStatusO').prop('disabled', true).trigger('chosen:updated');
			}
			else
			{	
				<#if proposalInitiation?? && proposalInitiation.legalApprovalStatus?? && proposalInitiation.legalApprovalStatus gt 0>
					$j('#confirmProjectEnable').removeClass("disabled");
				<#else>
...
<#elseif project.processType?? && (project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].END_MARKET_EU_OFFLINE.getId() || project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_EU_OFFLINE.getId())>
	...
	<#--<#if isAdminUser && proposalInitiation?? && proposalInitiation.legalApprovalStatus?? && proposalInitiation.legalApprovalStatus gt 0>
		<a href="javascript:void(0);" style="margin-left: 10px;" onclick="return resetProposal();" class="publish-details">Reset</a>	
	</#if>-->
		...
		$j(document).ready(function() {
			...
			{	
				<#if proposalInitiation?? && proposalInitiation.legalApprovalStatus?? && proposalInitiation.legalApprovalStatus gt 0 && proposalInitiation.legalApprovalDate?? && attachmentMap?? && attachmentMap.get(proposalLegalApprovalID)?? && proposalInitiation.proposalLegalApprover?? && proposalInitiation.proposalLegalApprover gt 0 && attachmentMap?? && attachmentMap.get(proposalID)??>
					$j('#confirmProjectEnable').removeClass("disabled");
				<#else>
					//$j('#confirmProjectEnable').addClass("disabled");
				</#if>
			...
			<#elseif proposalInitiation?? && proposalInitiation.legalApprovalStatus?? && proposalInitiation.legalApprovalStatus gt 0 && proposalInitiation.legalApprovalDate?? && attachmentMap?? && attachmentMap.get(proposalLegalApprovalID)?? && proposalInitiation.proposalLegalApprover?? && proposalInitiation.proposalLegalApprover gt 0 && attachmentMap?? && attachmentMap.get(proposalID)??>
				$j('#confirmProjectEnable').removeClass("disabled");
			<#else>
				//$j('#confirmProjectEnable').addClass("disabled");
			</#if>
...
<div id="emailNotification" style="display:none">
    <form id="email-notification-form" action="<@s.url value="/new-synchro/proposal-details!sendNotification.jspa"/>" method="post" enctype="multipart/form-data" class="j-form-popup">
	...
	<!--amit-->
			...
			<div class="region-inner">
					...
					<div class="col-lg-6">
						...
						<#else>
							<select data-placeholder="Select Methodology" class="chosen-select" id = "methodologyDetails" disabled name = "methodologyDetails" >
						</#if>
						  <option value=""></option>
...
<input type="text"  name="multiBrandStudyText" placeholder="Enter the Brands"" id="multiBrandStudyText"  onfocus="this.placeholder='' " <#if project?? && project.multiBrandStudyText??> value="${project.multiBrandStudyText}" </#if> onblur="this.placeholder = 'Enter the Brands' "/>
							...
							<#if endMarketDetails??>
							    <#list endMarketDetails as emd >
								    <#if emd?? && emd.getIsFundingMarket()?? && emd.getIsFundingMarket()>
									    <option value="${emd.getEndMarketID()}" selected>${endMarkets.get(emd.getEndMarketID()?int)}</option>
									<#else>
										<#if emd?? && emd.getEndMarketID()?? && endMarkets.get(emd.getEndMarketID()?int)??>
											<option value="${emd.getEndMarketID()}">${endMarkets.get(emd.getEndMarketID()?int)}</option>
										</#if>	
									</#if>
								</#list>
			...
			<div class="region-inner">
				...
				<div class="form-select_div form-select_div_project_completion region-inner">
					...
					<label>Legal Approver's Name</label>
					<div class="legalAppAttach clearfix">
						<#if (isAdminUser || isSynchroSystemOwner) || (project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_EU_OFFLINE.getId() || project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].END_MARKET_EU_OFFLINE.getId())>
							...
							<#if projectInitiation.getBriefLegalApproverOffline()??>
								<input type="text"  id="legalApproverName" placeholder="Enter Legal Approver’s Name" name="briefLegalApproverOffline"  value="${projectInitiation.getBriefLegalApproverOffline()}"  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"/>
							<#else>
								...
								<select data-placeholder="Select Legal Approver"  class="chosen-select" id = "briefLegalApprover" name='briefLegalApprover'><span class="jive-error-message full-width" id="legalApproverError" style="display:none">Please enter Legal Approver's Name</span>
								...
								<#else>
								 ...
								 <#else>
								<input type="text" id="legalApproverName" placeholder="Select Legal Approver" name="briefLegalApproverOffline"  value=""  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"/>
								
								 </#if>
								
							</#if>	
							 <span class="jive-error-message full-width" id="briefLegalApproverError" style="display:none">Please enter Legal Approver's Name</span>
						<#else>
							...
							<input type="text"  name="legalApproverName" value="${legalApproverName}" id="legalApproverName"  disabled  onfocus="this.placeholder = ''" onblur="this.placeholder = 'Enter Legal Approver’s Name'"/>
						</#if>
...
<form method="POST" name="sendForApprovalForm" id="sendForApprovalForm" action="/new-synchro/proposal-details!updateSendForApproval.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
	<input type="hidden" id="proposalLegalApproverHidden" name="proposalLegalApproverHidden" value="">
</form>

<form method="POST" name="sendReminderForm" id="sendReminderForm" action="/new-synchro/proposal-details!updateSendReminder.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
</form>

<form method="POST" name="needDiscussionForm" id="needDiscussionForm" action="/new-synchro/proposal-details!updateNeedsDisussion.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
</form>
<form method="POST" name="confirmLegalApprovalSubmissionForm" id="confirmLegalApprovalSubmissionForm" action="/new-synchro/proposal-details!confirmLegalApprovalSubmission.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
	<input type="hidden" name="legalApprovalStatus" id="legalApprovalStatus" value="">
	
</form>

<form method="POST" name="cancelProj" id="cancelProj" action="/new-synchro/proposal-details!cancelProject.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
</form>

<form method="POST" name="enableProj" id="enableProj" action="/new-synchro/proposal-details!enableProject.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
</form>
<form method="POST" name="resetProposalForm" id="resetProposalForm" action="/new-synchro/proposal-details!resetProposal.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
</form>

<form method="POST" name="resetPIBForm" id="resetPIBForm" action="/new-synchro/proposal-details!resetPIB.jspa"  >
    <input type="hidden" name="projectID" value="${projectID?c}">
</form>
...
<#if proposalInitiation?? && proposalInitiation.legalApprovalDate?? >		
    var _legalApprovalDate = "${proposalInitiation.legalApprovalDate?string('dd/MM/yyyy')}";
...
function validatePIBFormFields1(continueVar)
{
	...
	if(legalSignOffRequired)
	{
	 ...
	 /*	$j("#legalSignOffReqValue").val('yes');
		var proposalForm = jQuery("#proposal-form");
		proposalForm.submit();
		*/
	...
	console.log("validatePIBFormFields");
		...
		*/
		...
		if(continueVar!="" && $j("#processType").val()=="offLine")
		{
			<#if attachmentMap?? && attachmentMap.get(proposalLegalApprovalID)?? >
			 <#else>
				
				$j("#attachApprovalEmailError").show();
			...
			var legalApprovalStatusO=$j("#legalApprovalStatusO");
			if(legalApprovalStatusO.val() != null && $j.trim(legalApprovalStatusO.val())=="")
			{
			     var isDisabled = $j("#legalApprovalStatusO").prop("disabled");
				   if(isDisabled)
				   {
				     $j("#TPDStatusError").hide();
					 $j("#attachApprovalEmailError").hide();
				   }
				   else
				   {
				     $j("#TPDStatusError").show();
			        error=true;
			...
			var legalApprovalDate=$j("#legalApprovalDate");
			
			  if(legalApprovalDate.val()!=null &&$j.trim(legalApprovalDate.val())=="")
			  {
			      $j("#legalApprovalDateError").show();
			      error=true;
		 ...
		 else
        {
		  // for online process
			...
			if( (proposalLegalApprover.val() != null && $j.trim(proposalLegalApprover.val())=="")  || (briefLegalApprover.val() != null && $j.trim(briefLegalApprover.val())==""))
			{
			         
			     var isDisabled = $j("#proposalLegalApprover").prop("disabled");
			     
				    // if component is already disabled....
				   if(isDisabled)
				   {
		    ...
		    var isDisabled = $j("#sendForApprovalEnable").hasClass("disabled");
			
			var isDisabledbriefLegalApprover = $j("#proposalLegalApprover").prop("disabled");
			if(isDisabled==false && isDisabledbriefLegalApprover==false)
			{
			   dialog({
					title:"",
					html:"<i class=' positionSet'></i><p>As an approval on TPD status is still pending, you cannot move to the next stage.</p>",
		    ...
		    var adminConfirmSubmisssion=$j("#adminConfirmSubmisssion").prop("disabled");
			var legalApprovalStatusO= $j("#legalApprovalStatusO").val();
			
			 
			
			if( isDisabledbriefLegalApprover==true && adminConfirmSubmisssion==false  && legalApprovalStatusO=="")
			{
		...
		// check the visibility of send reminder button
			if($j('#sendReminder').is(':visible'))
			{
			  <#if isProposalLegalApprover|| isAdminUser || isSynchroSystemOwner>
			     dialog({
					title:"",
					html:"<p>Please approve for project to Proceed. </p>",
...
function validatePIBFormFields(continueVar)
{
	...
	/*	if(proposal.val() != null && $j.trim(proposal.val())=="")
		{
			if(!error)
				proposal.focus();
			error = true;
...
<script type="text/javascript">
  ...
  function updateSendForApproval()
  {
	...
	FORM_DATA_PERSIST_HANDLER.handle({parentForm:$j("#proposal-form"),form:$j("#sendForApprovalForm"), projectID:${projectID?c}});
	/*var proposal = $j("textarea[name=proposal]");
	if(proposal.val() != null && $j.trim(proposal.val())=="")
	{
		dialog({
					title:"",
					html:"<p>Please select Project Proposal.</p>",

				});
				orangeBtn();
		return false;
	}
	var proposalLegalApprover= $j("input[name=proposalLegalApprover]");
	...
	var proposalLegalApproverText = $j('select[name="proposalLegalApprover"] option:selected').text();
	
	dialog({
				title:"",
				html:"<p>An e-mail notification for the Proposal has been sent to "+proposalLegalApproverText+" for approval.</p> <p>You can view the status of the approval through the current project page.</p>",
				 buttons:{
					"OK":function() {
  ...
  {	
	...
	if(legalApprovalStatus!=null && (legalApprovalStatus == -1 || legalApprovalStatus.trim()==""))
	{
		dialog({
					title:"",
					html:"<p>As an approval on TPD status is still pending, you cannot move to the next stage.</p>",
   ...
   function continueProject()
  {
	...
	
	{
		...
		<#if pibMethodologyWaiver?? && pibMethodologyWaiver.methodologyApprover??>
			...
			nextCheckError=confirmProposal();
		...
		<#else>
			 ...
			 // if methodlogy waiver not selected
			 if(!error && ($j("#deviationFromSM").val()=="-1"  || $j("#deviationFromSM").val()=="2")  )
			{
		...
		if(flag==true || nextCheckError==false)
		 {
		 ...
		 else if(nextCheckError=="undefined" || nextCheckError==undefined)
		 {
		 var proposalForm = jQuery("#proposal-form");
		 $j("#legalSignOffReqValue").val('yes');
  ...
  function confirmProposal()
  {
	...
	{
		<#if project.processType?? && project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_NON_EU.getId() && showRefProjectMessage=="yes" && project?? && project.euMarketConfirmation?? && project.euMarketConfirmation==1>
			var isRefSynCodeEmpty = validateReferenceSynchroCodes();
			if(isRefSynCodeEmpty=="yes")
			{ 
				dialog({
					title:"",
					html:"<p>Provide the SynchrO Code for the market study associated with this project. If you have not received the project code, reach out to the market contact for details.</p>",
					 buttons:{
						"OK":function() {   
		   ...
		   showError  = validatePIBFormFields1("continue");
...
$j("#brandSpecificStudy").change(function() {
	...
	function cancelProject()
  {
	
	{
		dialog({
					title:"",
					html:'<i class="warningErr"></i><p>Once you cancel the project, it will no longer be accessible.</p><p>Are you sure you want to cancel the project?</p>',
					buttons:{
					"YES":function() {
 ...
 function enableProject()
  ...
  function resetPIB()
  {
	...
	$j(document).ready(function() {
		...
		<#--  <#if isSynchroSystemOwner >
				$j('#sendForApprovalEnable').addClass("disabled");-->
		  <#if proposalInitiation?? && proposalInitiation.proposalLegalApprover?? && proposalInitiation.proposalLegalApprover gt 0 && attachmentMap?? && attachmentMap.get(proposalID)??>
				 var legalSignOffRequired=$j("#legalsignoffreq").prop("checked");
	...
	$j("#legalsignoffreq").change(function() {
		...
		{	
			...
			//$j('#confirmProjectEnable').addClass("disabled");	
			$j('#proposalLegalApprover').prop('disabled', false).trigger('chosen:updated');
			$j('#proposalLegalApprover').removeClass("disabled");	
			//$j('#confirmProjectEnable').addClass("disabled");
			...
			$j('#legalApprovalDate').prop('disabled', false);
			<#if project.processType?? && (project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].END_MARKET_EU_ONLINE.getId() || project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_EU_ONLINE.getId()) && attachmentMap?? && attachmentMap.get(proposalID)??>
	...
	$j("#proposalLegalApprover").change(function() {
		<#if project.processType?? && (project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].END_MARKET_EU_OFFLINE.getId() || project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_EU_OFFLINE.getId())>
			...
			<#if proposalInitiation?? && proposalInitiation.legalApprovalStatus?? && proposalInitiation.legalApprovalStatus gt 0 && proposalInitiation.legalApprovalDate?? && attachmentMap?? && attachmentMap.get(proposalLegalApprovalID)?? && proposalInitiation.proposalLegalApprover?? && proposalInitiation.proposalLegalApprover gt 0 && attachmentMap?? && attachmentMap.get(proposalID)?? >
				...
				$j("#proposalLegalApproverHidden").val(proposalLegalApprover);
  ...
  // change 473
   ...
   $j("#methodologyDetails").change(function() {
	...
	function dateCompare()
	{
		...
		var legalApprovalDateBrief = $j("#legalApprovalDateBrief").val();
		
		if(!compareDate(legalApprovalDateBrief, legalApprovalDate))
		{
		  
		 dialog({
				title:"",
				html:"<p>Please check the date you have selected.</p><p> <i>Date of Legal Approval</i> on Proposal cannot be prior to the <i>Date of Legal approval on Brief</i>.</p>",
				buttons:{
				"OK":function() {
	...
	<#if project.processType?? && (project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].END_MARKET_EU_ONLINE.getId() || project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_EU_ONLINE.getId()) >
		$j(document).ready(function() {
                var approvalSubmit = $j("#proposalLegalApprover").is(':disabled');
...
$j("#deviationFromSM").change(function()
{
   $j("#deviationFromSMError").hide();
...
$j("#legalApprovalStatusO").change(function() {
var legalApprovalStatusO=$j("#legalApprovalStatusO");
			if(legalApprovalStatusO.val() != null && $j.trim(legalApprovalStatusO.val())!="")
			{
	              $j("#TPDStatusError").hide();
