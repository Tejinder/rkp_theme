	...
	<#if !isBriefLegalApprover>
	 <div class="project_owner_names">
			...
			<select data-placeholder="Select Legal Approver" class="chosen-select" id = "briefLegalApprover" name='briefLegalApprover'  >
			 ...
			 <span class="jive-error-message" id="briefLegalApproverError" style="display:none">Please Select Brief Legal Approver</span>
		...
		<#if projectInitiation?? && projectInitiation.sendForApproval?? && projectInitiation.sendForApproval==1 >
			...
			<span class="approval-status popTpd">TPD Status: <sapn class="status-txt">Pending</span></span>
		...
		<div class="date-approval"><label>Date of Legal Approval</label> <input type="text"  name="legalApprovalDate" id="legalApprovalDate" <#if projectInitiation?? && projectInitiation.legalApprovalDate??> value="${projectInitiation.legalApprovalDate?string('dd/MM/yyyy')}" <#else>value="" </#if> disabled />
		</div>
	</#if>	
	<#if isBriefLegalApprover >
			...
			<#else>
			   ...
			   <span>OR</span>
				...
				<input type="button" name="" id="needsDiscussionButtonEnable"  onclick="updateNeedDiscussion();" value="Needs Discussion" class="save need-discussion btn-blue"/>
				  ...
				  <div  id="approverContinueButton" class="buttons legalappr_btmmrgin">
						...
						<div class="form-select_div">
							...
							<#else>
								...
								<#if statics['com.grail.synchro.util.SynchroUtils'].getCurrencyFields().get(statics['com.grail.synchro.util.SynchroUtils'].getUserLocalCurrency(user))?? >
									...
									<#if statics['com.grail.synchro.util.SynchroUtils'].getPreferredCurrenyTotalCost(project,user)?? >
										<input type="text" placeholder="0" id="prefCost" name="prefCost" class="text_field numericfield longField" value="${statics['com.grail.synchro.util.SynchroUtils'].getPreferredCurrenyTotalCost(project,user)?round}" disabled />
									<#else>
										<input type="text" placeholder="0" id="prefCost" name="prefCost" class="text_field numericfield longField" value="" disabled />
									</#if>
