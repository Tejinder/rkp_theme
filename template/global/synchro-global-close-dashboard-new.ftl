...
//        $j("#ascendingOrder").val("");
    ...
    {
        currPage = page;
        var url = '/new-synchro/global-dashboard-close-pagination.jspa'
        if(xhr != null)
            xhr.abort();
...
function processDownloadRequest(page, keyword, plimit, downloadReportType) {
	...
	$j("#result-filter-form").prop("action", "/new-synchro/global-dashboard-close-pagination!downloadReport.jspa");
	var filterForm = $j("#result-filter-form");
...
<!-- Audit Logs -->
	...
	<#assign i18CustomText>Global Close Projects</#assign>
