...
<div class="region-inner">
	<a class="" href='${JiveGlobals.getJiveProperty("synchro.brief.template.path"," /file/download/AgencyCostDetails-SingleMarketTemplate.xlsx")}'>Click here to download Project Brief template</a>
...
<#if project.processType?? && (project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].END_MARKET_EU_ONLINE.getId() || project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_EU_ONLINE.getId())>
    ...
    <#if isLegalApprover>
		...
		<div class="project_status_div">
		   ...
		   <#if (projectInitiation?? && projectInitiation.legalApprovalDate??) || ( projectInitiation?? && projectInitiation.needsDiscussion?? && projectInitiation.needsDiscussion==1 )>
	...
	<#if !isBriefLegalApprover>
	 ...
	 <div class="project_owner_names">
		...
		<#if projectInitiation?? && projectInitiation.sendForApproval?? && projectInitiation.sendForApproval==1 >
			...
			<span class="approval-status popTpd">TPD Status: <sapn class="status-txt">Pending</sapn></span>
		<#elseif projectInitiation?? && projectInitiation.legalApprovalStatus?? && (projectInitiation.legalApprovalStatus == 1)  >
			<span class="approval-status popTpd">TPD Status: <sapn class="status-txt">May have to be TPD Submitted</sapn></span>
		<#elseif projectInitiation?? && projectInitiation.legalApprovalStatus?? && (projectInitiation.legalApprovalStatus==2)  >
			<span class="approval-status popTpd">TPD Status: <sapn class="status-txt">Does not have to be TPD Submitted</sapn></span>
		<#else>
	 ...
	 <div class="project_owner_names">
			...
			<#else>
				<#if projectInitiation.getBriefLegalApproverOffline()??>
						<input type="text" id="briefLegalApprover" disabled placeholder="Select Legal Approver" name="briefLegalApproverOffline"  value="${projectInitiation.getBriefLegalApproverOffline()}"  />
					<#else>
						<input type="text" id="briefLegalApprover" disabled placeholder="Select Legal Approver" name="briefLegalApproverOffline"  value=""  />
					</#if> 
	...
	<#assign counter = counter + 1 />
	 ...
	 <#if methBriefException == "yes">
		...
		<#elseif projectInitiation?? && projectInitiation.needsDiscussion?? && projectInitiation.needsDiscussion==1 >
			<div class="legal-checkbox" id="legal-checkbox"><input disabled type="checkbox" name="legalsignoffreq" id="legalsignoffreq"  <#if projectInitiation?? && projectInitiation.legalSignOffRequired?? && projectInitiation.legalSignOffRequired==1> checked="true"</#if>>This project does not require legal sign-off on Brief.<br></div>
		<#elseif projectInitiation?? && projectInitiation.legalApprovalStatus?? && projectInitiation.legalApprovalStatus gt 0>
			<div class="legal-checkbox" id="legal-checkbox"><input type="checkbox" name="legalsignoffreq" disabled id="legalsignoffreq" <#if projectInitiation?? && projectInitiation.legalSignOffRequired?? && projectInitiation.legalSignOffRequired==1> checked="true"</#if> >This project does not require legal sign-off on Brief.<br></div>
		<#else>
			<div class="legal-checkbox"  id="legal-checkbox"><input type="checkbox" disabled name="legalsignoffreq" id="legalsignoffreq"  <#if projectInitiation?? && projectInitiation.legalSignOffRequired?? && projectInitiation.legalSignOffRequired==1> checked="true"</#if>>This project does not require legal sign-off on Brief.<br></div>
		</#if>
...
<script type="text/javascript">
    ...
    $j(document).ready(function(){
		...
		$j("#needsDiscussionButtonEnable").addClass("disabled");
         $j("#legalApprovalStatusO").addClass("disabled")		;
...
<div id="initiateKantarWaiver" style="display:none" class="j-form-popup">
    <script type="text/javascript">
        $j(document).ready(function(){
		  ...
		  <h3>Project Details Overview</h3>
			...
			<div class="region-inner">
				...
				<div class="col-lg-6">
					...
					<select data-placeholder="" class="chosen-select" id = "categoryType" disabled name="categoryType" multiple >
					  <option value=""></option>
			...
			<div class="region-inner">
				...
				<#assign selectedEndMarket= endMarketDetails.get(0).endMarketID />
					...
					<#if project.processType?? && project.processType ==  statics['com.grail.synchro.SynchroGlobal$ProjectProcessType'].GLOBAL_NON_EU.getId()>
						...
						<select data-placeholder="" class="chosen-select" id = "endMarkets" disabled name='endMarkets' multiple disabled >
						<option value=""></option>
			...
			<div class="region-inner">
				...
				<input type="text" id="startDate" name="startDate1" value="" disabled>
			...
			<div class="region-inner">
				...
				<input type="text" id="endDate" name="endDate1" value="" disabled>
			...
			<div class="region-inner">
					...
					<div class="col-lg-6">
						...
						<#else>
							<select data-placeholder="" disabled class="chosen-select" id = "methodologyDetails" name = "methodologyDetails" >
						</#if>
						  <option value=""></option>
				...
				<div class="region-inner">
					...
					<select data-placeholder="Select" class="chosen-select" id = "methWaiverReq" disabled name="methWaiverReq" <#if project.methWaiverReq?? && project.methWaiverReq==1 && pibMethodologyWaiver?? && pibMethodologyWaiver.projectID?? >disabled</#if>   onchange="checkMethDeviation()">
						<option  value="-1" <#if project.methWaiverReq?? && project.methWaiverReq==-1 > selected </#if>></option>
						<option  value="1" <#if project.methWaiverReq?? && project.methWaiverReq==1 > selected </#if>>Yes</option>
						<option  value="2" <#if project.methWaiverReq?? && project.methWaiverReq==2 > selected </#if>>No</option>
						<option  value="3" <#if project.methWaiverReq?? && project.methWaiverReq==3 > selected </#if>>I don’t know yet</option>
				...
				<label>Cost Details</label>
					...
					<div class="region-inner">
						...
						<div class="form-select_div">
							...
							<input type="text"  id="enter_cost" name="agencyCosts" firstRow="yes" disabled Placeholder="Enter Cost" class="text_field  longField total-expense" value="${projectCostDetail.getEstimatedCost()}" />
			...
			<div class="region-inner two-column">
				...
				<div class="form-select_div second-column" id="fundingMarketsDiv" <#if project.endMarketFunding?? && project.endMarketFunding==1> style="display:block" <#else>style="display:none"</#if>>
					...
					<#else>
						...
						<select data-placeholder="Select end market" class="chosen-select" id = "fundingMarkets" disabled name="fundingMarkets" >
							<option value=""></option>
							<#if endMarketDetails??>
							    <#list endMarketDetails as emd >
								    <#if emd?? && emd.getIsFundingMarket()?? && emd.getIsFundingMarket()>
...
<script>
 $j(document).ready(function(){
    ...
    $j('#legalApprovalStatusO_chosen').prop('disabled', true);
    $j('#legalApprovalStatusO').prop('disabled',true);
 });
