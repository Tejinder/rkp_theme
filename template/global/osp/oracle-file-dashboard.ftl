...
<script language="JavaScript" type="text/javascript" src="<@s.url value='${themePath}/js/synchro/ajaxfileupload.js'/>"></script>
			...
			<div class="ui-dialog ui-widget ui-widget-content ui-corner-all" tabindex="-1" role="dialog" aria-labelledby="ui-id-1">
				...
				<div id="project-create-options" class="ui-dialog-content ui-widget-content" scrolltop="0" scrollleft="0">
				 ...
				 <div class="cont-popFooter">
				  <img alt="sorting" src="/themes/rkp_theme/images/osp/oracle/icon-lock.png" />Only people with ORACLE access have permission to view this link 
				 </div>
				</div>
			</div>
		</div>
			...
			<div class="ui-dialog ui-widget ui-widget-content ui-corner-all" tabindex="-1" role="dialog" aria-labelledby="ui-id-1">
				...
				<div id="project-create-options" class="ui-dialog-content ui-widget-content" scrolltop="0" scrollleft="0">
				 <div class="cont-popupFields">
				  ...
				  <form id="folder-rename-form" action="/oracle/oracle-folder-dashboard!renameFolder.jspa" method="POST">
		...
		<div class="popupCont" id='new_popup'>
			<div class="ui-dialog ui-widget ui-widget-content ui-corner-all" tabindex="-1" role="dialog" aria-labelledby="ui-id-1">
				...
				<div id="project-create-options" class="ui-dialog-content ui-widget-content" scrolltop="0" scrollleft="0">
				 ...
				 <label>Folder Name </label>
				  <form id="folder-create-form" action="/oracle/oracle-folder-dashboard!createNewFolder.jspa" method="POST">
		...
		<#assign canEditOSP = statics['com.grail.synchro.util.SynchroPermHelper'].canEditOSPOraclePortal(user) />
		...
		<div class="cont-gray">
			<div class="breadcrumb">
				...
				<#if sortBy??>
					<#if currPage??>
						<#if sortOrder??>
							<a href="/oracle/oracle-folder-dashboard.jspa?tileID=${ospTile.getTileId()}&currPage=${currPage}&sortField=${sortBy}&sortOrder=${sortOrder}"><span>${ospTile.getTileName()}</span></a>
						<#else>
							<a href="/oracle/oracle-folder-dashboard.jspa?tileID=${ospTile.getTileId()}&currPage=${currPage}&sortField=${sortBy}"><span>${ospTile.getTileName()}</span></a>
						</#if>
						
					<#else>
						<#if sortOrder??>
							<a href="/oracle/oracle-folder-dashboard.jspa?tileID=${ospTile.getTileId()}&sortField=${sortBy}&sortOrder=${sortOrder}"><span>${ospTile.getTileName()}</span></a>
						<#else>
							<a href="/oracle/oracle-folder-dashboard.jspa?tileID=${ospTile.getTileId()}&sortField=${sortBy}"><span>${ospTile.getTileName()}</span></a>
						</#if>
						
					</#if>	
				<#else>
					<#if currPage??>
						<a href="/oracle/oracle-folder-dashboard.jspa?tileID=${ospTile.getTileId()}&currPage=${currPage}"><span>${ospTile.getTileName()}</span></a>
					<#else>
						<a href="/oracle/oracle-folder-dashboard.jspa?tileID=${ospTile.getTileId()}"><span>${ospTile.getTileName()}</span></a>
					</#if>	
				</#if>	
	...
	<form id="file-attachment-form" action="/oracle/oracle-file-dashboard!download.jspa" method="POST" class="result-form-popup ">
	...
	<form id="file-delete-form" action="/oracle/oracle-file-dashboard!delete.jspa" method="POST" class="result-form-popup ">
...
function processPaginationRequest(page, keyword, plimit) {
	...
	var url = '/oracle/oracle-file-dashboard-pagination.jspa';
...
//debugger;
    ...
    $j(document).ready(function(){
        ...
        <#if sortBy?? && sortBy!="">
            ...
            formUrl:"<@s.url value='/oracle/oracle-file-dashboard!addAttachment.jspa'/>",
