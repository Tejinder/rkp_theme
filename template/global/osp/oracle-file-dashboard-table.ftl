					...
					<#if oracleFileList?? && oracleFileList?size gt 0 >
						<#list oracleFileList as oracleFile>
							<tr class="table-row">
								...
								<td class="table-col cont-name"><span><a id = "${oracleFile.getAttachmentId()?c}" href="<@s.url value="/servlet/JiveServlet/download/${oracleFile.getObjectId()?c}-${oracleFile.getAttachmentId()?c}/${oracleFile.getFileName()?url}" />" >${oracleFile.getFileName()}</a></span></td>
								
								
								<td class="table-col cont-modified"><span>${oracleFile.getModifiedDateString()}</span></td>
								<td class="table-col cont-file-size"><span>${oracleFile.getFileSizeString()}</span></td>
							</tr>
						</#list>
					</#if>
				</tbody>
