...
<script type="text/javascript">
    function changeStatus(status) {
        $j.ajax({
            url: "<@s.url value="/kantar/status!update.jspa"/>",
            type: 'POST',
            data: {id:${id},status:status},
            success: function(data, test) {
                window.location.href = "<@s.url value="/kantar/status!input.jspa?id=${id}"/>";
            },
            error: function() {
    ...
    <div class="research_content">
        ...
        <div class="change-status-main">
            <div class="change-status">
                ...
                <ul>
                    <li class="status">
                        ...
                        <span class="message">${statics['com.grail.kantar.util.KantarGlobals$KantarBriefTemplateStatusType'].IN_PROGRESS.toString()?upper_case}</span>
                    </li>
                    <li class="separator <#if status &gt;= 2 && status &lt; 3>fill</#if>"></li>
                    <li class="status">
                        <span class='progress last <#if status &gt;= 2 && status &lt; 3>completed</#if>'></span>
                        <span class="message">${statics['com.grail.kantar.util.KantarGlobals$KantarBriefTemplateStatusType'].COMPLETED.toString()?upper_case}</span>
                    </li>
                    <#--<li class="separator <#if status &gt;= 2>fill</#if>"></li>-->
                    <#--<li class="status">-->
                        <#--<span class='progress last <#if status &gt;= 3>completed<#elseif status = 2>active</#if>'></span>-->
                        <#--<span class="message">${statics['com.grail.kantar.util.KantarGlobals$KantarBriefTemplateStatusType'].CANCELLED.toString()?upper_case}</span>-->
                    <#--</li>-->
                </ul>
            </div>
            <div class="change-status-sub">
                ...
                <#assign statusTypes = statics['com.grail.kantar.util.KantarGlobals$KantarBriefTemplateStatusType'].values()>
