...
function validatePost(isSubmit, ignoreBody, editorId) {
    ...
    if(month != null || year != null) {
        ...
        if (typeof(month) != "undefined" && trimString(month.value) != ''
            && typeof(year) != "undefined" && trimString(year.value) != '') {
            if(trimString(year.value) == 'NA' && trimString(month.value) != 'NA') {
                // display alert
                var t = document.getElementById('post-error-table');
                if (t) {
                    t.style.display = "block";
                    scroll(0,0);
                    t = document.getElementById('post-error-year-month');
                    if (t) {
                        t.style.display = "block";
                        scroll(0,0);
                    }
                ...
                var t = document.getElementById('post-error-year-month');
                if (t) {
                    t.style.display = "none";
                }
            }
...
//        rte.setHTML(rte.getHTML());
    ...
    if (autoSave && isSubmit) {
        $j(window).unbind('unload', window.autoSave.doDiscard);
    }
